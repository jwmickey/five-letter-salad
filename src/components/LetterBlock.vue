...
<template>
  <div
    ...
    :class="{
      'text-gray-900': true,
      'bg-gray-600': correct === false && inWord === false && (correct !== undefined || inWord !== undefined),
...
.bg-correct-500.letter-revealing {
  animation: letterReveal 0.6s ease-in-out forwards, correctGlow 0.8s ease-in-out;
}
...
.bg-inword-300.letter-revealing {
  animation: letterReveal 0.6s ease-in-out forwards, inWordPulse 0.8s ease-in-out;
}
